...
class TestProgrammingReader:
    def test_file_not_found(self):
        with pytest.raises(FileNotFoundError):
            progr.ProgrammingReader(Path("C:/badpath.py"))

    def test_file_not_code(self):
        with pytest.raises(ValueError):
            progr.ProgrammingReader(
                Path("tests/testdata/test_fas_programming_reader/sampleText1.txt")
            )
    ...
    def test_unknown_file_extension(self):
        with pytest.raises(ValueError):
            progr.ProgrammingReader(
                Path(
                    "tests/testdata/test_fas_programming_reader/sampleUnkownExtension.xyz"
                )
            )

    def test_empty_file(self):
        programReader = progr.ProgrammingReader(
            Path("tests/testdata/test_fas_programming_reader/sampleEmptyFile.py")
        )
    ...
    def test_file_with_no_imports(self):
        programReader = progr.ProgrammingReader(
            Path("tests/testdata/test_fas_programming_reader/sampleNoImports.py")
        )
        assert programReader.filetype == "python"
        assert programReader.libraries == []
    ...
    def test_java_files(self):
        programReader = progr.ProgrammingReader(
            Path(r"tests/testdata/test_fas_programming_reader/sampleJavaFile.java")
        )
    ...
    def test_javascript_sample_file(self):
        ...
        programReader = progr.ProgrammingReader(
            Path("tests/testdata/test_fas_programming_reader/sampleJavaScriptFile.js")
        )
    ...
    def test_pascal_files(self):
        ...
        programReader = progr.ProgrammingReader(
            Path("tests/testdata/test_fas_programming_reader/samplePascalFile.pp")
        )
    ...
    def test_perl_files(self):
        ...
        programReader = progr.ProgrammingReader(
            Path("tests/testdata/test_fas_programming_reader/samplePerlFile.pl")
        )
    ...
    def test_Python_files(self):
        programReader = progr.ProgrammingReader(
            Path("tests/testdata/test_fas_programming_reader/samplePythonFile.py")
        )
        assert programReader.filetype == "python"
        assert len(programReader.libraries) > 0
    ...
    def test_go_files(self):
        ...
        programReader = progr.ProgrammingReader(
            Path("tests/testdata/test_fas_programming_reader/sampleGoFile.go")
        )
        ...
        def test_php_files(self):
            ...
            programReader = progr.ProgrammingReader(
                Path("tests/testdata/test_fas_programming_reader/samplePHPFile.php")
            )
            ...
            libs = [normslashes(lib) for lib in programReader.libraries]
            assert normslashes("Symfony\\Component\\HttpFoundation\\Request") in libs
            assert normslashes("Symfony\\Component\\HttpFoundation\\Response") in libs
            assert normslashes("Doctrine\\ORM\\EntityManager") in libs
            assert normslashes("vendor\\autoload.php") in libs

            # Should NOT contain relative paths
            assert normslashes("./local/helper.php") not in libs
            assert normslashes("../parent/config.php") not in libs
            assert not any(lib.startswith(("./", "../")) for lib in libs)

    def test_rust_files(self):
        """Test Rust extracts external crates and filters internal references"""
        programReader = progr.ProgrammingReader(
            Path("tests/testdata/test_fas_programming_reader/sampleRustFile.rs")
        )

        assert programReader.filetype == "rust"

        # Should extract external crates
        assert "std" in programReader.libraries or any(
            "std" in lib for lib in programReader.libraries
        )
        assert "serde" in programReader.libraries or any(
            "serde" in lib for lib in programReader.libraries
        )
        assert "tokio" in programReader.libraries or any(
            "tokio" in lib for lib in programReader.libraries
        )
    ...
    def test_ruby_files(self):
        ...
        programReader = progr.ProgrammingReader(
            Path("tests/testdata/test_fas_programming_reader/sampleRubyFile.rb")
        )
    ...
    def test_typescript_files(self):
        programReader = progr.ProgrammingReader(
            Path("tests/testdata/test_fas_programming_reader/sampleTypeScriptFile.ts")
        )
    ...
    def test_visualbasic_files(self):
        programReader = progr.ProgrammingReader(
            Path("tests/testdata/test_fas_programming_reader/sampleVisualBasicFile.vb")
        )
